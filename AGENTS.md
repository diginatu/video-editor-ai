...
- Stage 2 captions are chunked on GiNZA bunsetsu units with bunsetsu-level timing (`end = min(start+0.02s, next start)`), split on silence gaps and keep-boundary crossings; defaults are 12 bunsetsu, 4.0 seconds max, minimum 3 bunsetsu, min duration 1.5s, and silence flush at 1.5s.
